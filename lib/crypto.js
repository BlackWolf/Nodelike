...
function Hash(algorithm, isHmac, secret) {
	...
	if (isHmac) {
		...
		switch (algorithm) {
			case 'md5': this.crypto = CryptoJS.algo.MD5.create();
			case 'sha1': this.crypto = CryptoJS.algo.SHA1.create();
			case 'sha256': this.crypto = CryptoJS.algo.SHA256.create();
			case 'sha512': this.crypto = CryptoJS.algo.SHA512.create();
		}
	}
}
